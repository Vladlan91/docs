...
AI Agents in Revol are intelligent assistants that handle customer interactions across multiple channels — your website widget, phone calls, Telegram, WhatsApp, Viber, and Instagram. Each agent has its own personality, knowledge base, conversation workflow, and set of tools.
...
**Step 2** — select a use case: Customer Support, Outbound Sales, Lead Qualification, Answering Service, Consultation Booking, Client Intake, Service Recommendations, Scheduling, Billing Inquiries, Project Updates, Resource Library, Learning & Development, or Other.
...
- **Preview** — open widget as visitors see it
...
Two-column layout with auto-save (1.5s debounce).
...
Edges connect nodes and control message routing. Each edge has a **target node**, a **condition type**, and a **priority**. The workflow evaluates edges from highest to lowest priority — **the first match wins**.
...
Enter comma-separated keywords. The workflow checks if the visitor's message contains any of them using case-insensitive word matching.
...
The edge always fires. Used for unconditional connections — the default in the pre-built workflow where Start routes to all three system agents.

#### Fallback Condition

Routes only when no higher-priority edge matched. Always add a fallback edge from routing nodes to avoid dead ends.
...
| **Viber** | Viber integration | Viber bot |
